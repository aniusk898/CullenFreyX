...
The **sample skewness** formula is used to measure how symmetrical your data is. The formula is:

$$
\text{Sample Skewness} = \frac{\sum_{i=1}^{n} \left( \frac{x_i - \bar{x}}{s} \right)^3}{n}
$$
...
This formula gives you an idea of whether your data is skewed to the left or right, but it can be a little off for smaller datasets since it doesn’t account for bias. That's why we sometimes apply corrections when the sample size is small [@joanes1998comparing].
...
$$
\text{Sample Kurtosis} = \frac{\sum_{i=1}^{n} \left( \frac{x_i - \bar{x}}{s} \right)^4}{n} - 3
$$

The `-3` at the end is to adjust the value so that the kurtosis of a normal distribution equals 0 (this is called **excess kurtosis**). Like skewness, **sample kurtosis** might be biased in small samples [@decarlo1997].
...
$$
\text{Unbiased Skewness} = \frac{\sqrt{n(n-1)}}{n-2} \cdot \text{Sample Skewness}
$$
